...
| 持久存储 | 追加式 JSONL、SQLite WASM | 保存权威 Thread 历史、增量 Checkpoint、查询投影、记忆和审计记录。 |
| 检索 | SQLite FTS5、Orama、ONNX Runtime、Hugging Face Tokenizer | 为旧 Thread 证据与长期记忆提供关键词与语义混合检索。 |
...
4. 使用确定性 Working Checkpoint、受限近期工作集、相关旧 Thread 证据、适用规则和检索记忆构建分层模型上下文。
5. 只向模型提供本步骤允许使用的能力。
...
工作区记账在高频路径上采用增量方式：文件工具只更新实际写入目标的已验证 Manifest；在 Git 仓库中，命令审计通过 Git 收集已跟踪、暂存、未暂存、命令期间提交、未跟踪及有意义的 ignored 候选，再只计算这些路径的哈希；依赖、缓存和构建产物等 ignored 目录仍整树剪枝。非 Git 工作区继续使用完整文件系统快照作为兼容回退。完整一致性扫描只在持久 Checkpoint 或最终交付边界执行。
...
短命令使用单个同步动作；长时间任务使用相互独立的启动、轮询与取消动作，并绑定到发起它的 Thread 和 Agent。独立的扁平 Schema 避免依赖部分供应商不稳定支持的联合 Schema，同时保留统一超时预算、有界输出、进程树终止，以及任务完成前必须观察终态的约束。
...
Checkpoint 与投影用于减少恢复工作，增量 Checkpoint 则降低写入成本；它们都不会替代事件历史。新的保存会针对 Journal 紧邻的上一序列追加一个受限 Delta：变化的设置、新追加消息、更新后的文件观察、新增变更与命令记录，以及只能前移的压缩状态。回合、任务、Plan、审批与执行中调整仍由事件决定，Checkpoint Delta 不能引入或抹除这些状态。每个 Delta 都经过 Schema 与大小校验，并绑定精确的基准序列；遇到并发或分叉追加时会拒绝，而不是猜测性合并。

Resume 同时兼容这些增量记录与旧版全量状态快照。它按 Journal 顺序重建状态、回放后续事件、校验持久身份，并在需要时依据 Journal 修复过期 SQLite 投影。这样既保持旧 Thread 可恢复，也避免长任务反复序列化不断增长的完整状态。
...
完整对话保留在 Thread 权威事件历史中。每次模型请求接收三个互补且有界的层次：

1. **Working Checkpoint。** Runtime 确定性投影当前目标与约束、执行身份、对话与压缩计数、近期文件观察、变更、命令结果，以及活跃 Plan 或任务状态。它是精简的恢复地图，不是模型撰写的 Journal 替代品。
2. **近期工作集。** 最新累计工作摘要与最近一段消息在本地硬上限内原样携带；系统契约、当前环境与实时控制状态和该层一起组装。
3. **相关历史。** 只有早于近期工作集边界的证据可以进入检索。先前用户请求、可见的模型工作、文件工具返回的代码片段，以及其他工具或命令证据会被分块，并依据当前请求、活跃任务和近期对话排序；少量去重后的结果作为不可信证据回到 Prompt。

这种拆分让即时任务状态保持确定性，保留局部连续性，同时不必在每次请求中重复发送完整 Transcript。Working Checkpoint 与检索索引都只是派生辅助数据；更新或查询失败不会取代 Journal，也不会阻塞原本有效的请求。

模型通过专用上下文动作生成累计工作摘要。Runtime 会限制长度、脱敏，并验证压缩边界只能向前，然后把两者一起持久化。单次请求过大时可以临时省略较旧活跃消息，但不会重写正式摘要或压缩边界。

上下文压力逐级处理：正常运行、提示考虑压缩、强制下一步压缩，以及在超过配置边界前自动插入压缩请求。本地控制使用字符预算保证确定性；UI 中 Token 是估算值，除非该数据由供应商实际返回。

思考强度只扩大本地步骤预算，所有强度共用同一个配置上下文与压缩预算。压力以配置容量和近期消息有效容量中的较小值计算，从而在滚动工作集静默省略历史前触发压缩。这些限制是 Runtime 执行保护，不代表供应商模型本身的上下文窗口。

Thread 证据检索同时按规范化工作区身份与精确 Thread 身份隔离。因此，即使父子 Agent 操作同一工作区，父 Thread 与每个子 Thread 也拥有不同的候选集合。供应商隐藏 Thinking 永不进入索引；模型证据只包含可见回答文本和显式工具请求。Working Checkpoint 或证据持久化前会先做秘密脱敏，检索材料回到上下文后仍标记为不可信数据。

### 混合检索与长期记忆

在派生检索平面内，SQLite FTS5 是 Thread 证据的权威关键词索引，即使没有下载模型也保持可用。语义排序属于可选增强：本地多语言 ONNX 模型生成 Embedding，持久向量以有界批次补齐，内存中的 Orama 索引缓存兼容向量并完成排序。关键词与语义候选会结合重要性、时间接近度进行融合，再去重为少量 Top 结果。

Orama 缓存带代际校验且可以完全重建。Embedding 缺失、不兼容、损坏或不可用时，本进程禁用语义路径并继续使用 SQLite FTS5。向量故障可能降低相关性，但不能丢失权威对话状态，也不能扩大 Thread 边界。

长期记忆保存作用域绑定到规范化工作区的短原子事实，包括偏好、约定、架构、决策和环境信息。

新的原子事实可以在不预先检索的情况下批量提出，和现有活跃事实完全相同的内容会成为 no-op；修订与删除仍需先检索到精确的持久记忆身份。所有变更都在回合内暂存，只有回合到达允许的成功边界时才原子提交。被替代和遗忘的事实保留足够审计历史，但不再参与活跃检索。
...
SWE-bench 适配层在任务容器之外沿用相同恢复原则。自动重试只允许发生在 `agent.run` 之前的环境启动或 Agent 安装超时；这类重试不会消耗模型请求，也不会为失败的安装尝试生成 Checkpoint。Agent 自身超时和非零退出不会获得新的完整解题预算。与此独立，Agent 执行一旦开始，其清理路径会把 EASY CODE 数据目录、工作区 Patch 与普通未跟踪文件捕获为带完整性校验、由启动器管理的 Generation；显式恢复 Harbor Job 时，可以在同一 Job 作用域内恢复匹配的 Generation。绑定身份包含题目指令、Harbor Trial 作用域、基础提交、安装包内容哈希、Embedding 模型 Manifest 哈希、Provider 端点身份、模型、模式与 Thinking 设置；任何不匹配、父 Thread 歧义、Manifest 损坏、基础提交变化、符号链接或特殊文件都会安全失败。任何 Generation 都不能用于另一道 SWE-bench 题目。主机只在基准盘上准备并校验一次固定的多语言 ONNX 资产，可信适配层再把它传入每个 Trial 的临时缓存并二次校验，使混合检索成为被评测配置的一部分，而不是依赖临时联网的可选副作用。启动器路径在 Harbor 展开固定任务定义时仍对其 Docker Compose 子进程可见，但不会被显式注入 Agent 进程环境；因此固定的数据集与 Compose 定义属于可信主机边界。

终端提供可观察视图，但日志不会反过来成为权威状态。`/changes`、`/commands`、`/permissions`、`/tasks`、`/agents`、`/context`、`/memory` 和 `/usage` 都是 Runtime 状态的只读投影。

Token 效率来自确定性 Working Checkpoint、带硬上限的近期工作集、选择性旧证据检索、模型控制的上下文压缩、有限长期记忆检索、小型 DAG/Artifact 引用、私有子 Agent 上下文、Auto 直接回答，以及不把完整子 Agent 日志和图片字节塞入父上下文。
...
- **混合检索提高相关性，但增加本地资源。** Embedding 缺失、不兼容或重建时仍可使用 SQLite FTS5。
