...
1. `ToolSource` 负责发现和生命周期；`ToolCatalog` 按确定顺序合并来源，并为一次模型请求发布不可变快照。
...
4. `tool.result` Journal 事件记录实际选中的工具/来源身份、Schema 与能力元数据哈希、目录版本和目录哈希；恢复与审计不依赖之后恰好处于活动状态的目录。

内置工具的可用范围集中在一张声明式策略表中。未知旧工具使用保守的主 Agent 兼容配置。动态外部工具必须携带由宿主维护的元数据；外部写入、进程、网络写入和破坏性效果在没有显式 Runtime 授权桥时失败关闭。模型可见名称或稳定身份冲突会在构建请求前被拒绝。

这只是**扩展接缝，不是 MCP 接入**。仓库当前没有 MCP SDK、MCP 传输、服务发现、MCP 配置、凭据交换或 Resource/Prompt 适配器。未来 MCP Client 可以实现 `ToolSource` 与授权桥，而无需在 Agent 循环里增加供应商专属分支；连接监管和信任配置仍由应用层负责。

### 6.2 内置文件安全

[工具注册表](../src/tools/registry.ts) 创建内置工具，Runtime 再按角色和模式过滤目录中的全部工具。工具同时包含模型可见的 JSON Schema、本地 Zod 校验和结构化结果/错误协议。富结果使用有界的中立内容联合类型，覆盖文本、结构化值、附件引用、资源引用和持久制品；可执行参数及权威证据仍与展示内容分离。
