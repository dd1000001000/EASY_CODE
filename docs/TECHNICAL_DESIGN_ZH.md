...
交互界面是结构化状态的投影，不是一组彼此独立的打印调用。UI 状态分别保存稳定会话头部、追加式 transcript、临时动态活动、完整保留的 Thinking 折叠条目、作为普通用户消息保存的 Adjustment、模态 overlay 和持久 composer。Reducer 式更新可以忽略过期的活动停止事件，并在渲染前限制任务、子 Agent、进度、图片和选择器摘要的行数。Thinking 正文与助手最终回答属于源内容而不是摘要，因此不受展示层行数限制。
...
| 动态状态区 | 在输入框上方只保存仍在运行的进度、活动和已用时间。Screen writer 只能清除并替换终端底部的这些行。 |
...
模态选择器的优先级高于普通动态状态；Dangerous full access 的持续警告属于安全界面元素，会保留在选择器上方或旁边。`/model`、命令审批、Plan 审核和 Resume 共用盒式 overlay 行为：选中行高亮，其他行弱化，按上/下方向键移动，按 Enter 确认，按 Esc 取消。菜单临时以 Raw Mode 独占 stdin，并在所有退出路径恢复此前的输入模式、光标和流状态。取消审批会映射为拒绝，以保持失败关闭。单一输入所有者也能防止后台子 Agent 或状态更新消费交互输入。模型请求进行期间，持久 composer 可以接收 steering 调整消息，同时继续响应私有折叠条目 toggle 与 `Ctrl+C` 取消信号。折叠条目渲染器可以接管物理 stdin 并暂时屏蔽 readline 的终端写入，但解码后的编辑输入仍转交给同一个已暂停的规范 composer，不会创建第二份草稿或提交队列；真正的模态选择器则在释放 stdin 后原样恢复文本、附件、光标、Raw Mode 和流状态。

点击切换 Thinking 和图片粘贴共用同一条有序私有终端通道。随附扩展只识别格式严格的 Thinking 标题，并要求其中的正十进制 ID 是安全整数。检测到正在运行的 EASY CODE shell execution 或用户明确启用后，扩展会提供链接。为了恢复扩展宿主重载时错过的 start 事件，只有激活前已经存在的终端才能通过严格标记进入恢复状态；下一次观测到 shell start/end 或终端关闭时，该状态会立即撤销。VS Code 默认在 Windows/Linux 上通过 `Ctrl+点击`、在 macOS 上通过 `Cmd+点击` 激活终端链接。激活时会把包含 Thinking ID 的固定、无换行 toggle 消息发回产生链接的终端；终端解析器会把它作为协议消费，不会让它进入 composer 文本。已安装旧客户端的兼容 payload 会归一化为相同操作。该协议不赋予 Esc 任何含义。

原生终端 scrollback 不能安全替换任意历史行，因此打开 Thinking 时会切换到由保留虚拟文档驱动的托管 alternate-screen transcript。每个 Thinking 节点都同时保留预览和完整正文。任一时刻最多展开一个节点：其完整正文会在同一逻辑文档位置替换预览，其他 Thinking 继续显示预览。关闭时在原位置恢复预览；选择其他标题则移动展开目标，不会复制正文。虚拟文档按照追加顺序投影一个边界明确的本轮 transcript 范围，其中包含初始用户请求、Adjustment、状态与工具条目、Thinking 节点和最终回答，同时排除之前轮次。主 scrollback 不会被改写，每条稳定内容仍只提交一次。

托管 transcript 把本轮完整内容与 Thinking 正文作为连续终端内容展示，不存在分页模型、正文行数上限、静默省略或 `truncated` 替代内容。短文档会直接底部对齐在 composer 上方；文档超过视窗时，所选标题固定在 transcript 顶部，Page Up/Page Down 只改变同一份保留内容的可见窗口。Alternate-screen writer 明确不启用 DEC 鼠标上报，因此保留终端原生拖选和复制；VS Code 的折叠激活继续使用独立的终端链接协议。普通方向键和编辑键仍属于 composer；终端尺寸变化后根据保留源文本重新换行。视图打开期间到达的新输出会立即追加到同一份本轮投影，并延迟到关闭时只向主 scrollback 物理提交一次；新的 Adjustment 以普通用户行加入。同一个规范 composer 在托管视图中保持活动，其文字、附件、光标、多行粘贴标记、图片捕获和 FIFO 提交都不会复制或丢失；关闭只切换投影，并恢复普通 readline 绘制和物理光标所有权。Thinking 的展开选择属于临时 UI 状态，不会写入 Thread 日志、模型上下文、短期上下文或长期记忆；保留的 Provider Thinking 与用户 Adjustment 数据仍然持久。`/thinking N`、`/adjustment N` 与 `Ctrl+T` 采用另一条只读路径，把完整保留内容提交到稳定 scrollback，同时保留 composer 草稿。

助手最终回答、已完成工具输出、命令输出和 Diff 都是普通 transcript 条目，不属于折叠内容。它们保持为追加式主 scrollback 内容，不受折叠视图或动态状态区的展示限制。命令输出捕获上限等源数据安全边界必须单独明确标注，不能伪装成 UI 预览。
...
Turn 中途调整由 Runtime 的 FIFO 管理，而不是开启第二条并发对话。每条文字/图片消息都会先独立写入日志，UI 才确认已排队。到达安全边界时，Runtime 对当前 FIFO 前缀取快照，并按顺序合并为一条用户消息交给模型；快照之后到达的条目留给下一边界。新消息只会取消当前 Provider attempt，不会取消整个 Turn。两个工具调用之间出现调整时，已经完成的结果继续有效，旧响应中尚未开始的调用先以 superseded 状态闭合，再追加调整消息，从而保持 assistant/tool 协议完整。最终输出前会停止新的输入接纳，并与持久 inbox 做原子检查，避免响应边界静默丢消息。调整只能改变任务指导，不能改变能力、审批、沙箱状态、工作区边界或任务归属。
...
事件日志覆盖用户与助手消息、排队/应用的 steering 条目及消费水位、结构化操作及结果、模式决策、Plan 审核、上下文压缩、任务迁移、子 Agent 生命周期、文件与命令审计，以及 Provider 返回的用量。稳定身份和顺序号使状态回放具有确定性，也使重复投影保持幂等。
