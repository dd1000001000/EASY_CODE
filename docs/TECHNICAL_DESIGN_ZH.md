...
Code 交付前，Runtime 检查后台命令、子 Agent 和 DAG 状态，再让微调后 Laya 对比用户需求与主 Agent 准备提交的最终回答。`CHALLENGE`，或分数低于默认 0.9 阈值的 `RELEASE`，会要求复查一次；该机会在 Resume 后继续有效。下一次交付继续通过原有完成条件和 reviewer。本地推理失败时会明确提示，并继续原有完成检查。模型、输入限制、配置和结果见[本地决策与训练](#61-实验性本地决策模型)。
...
随包提供的**微调后 Laya（joint-v2）** 从 **convaiinnovations/laya-multilingual** 微调而来。上游是基于 **mmBERT-base** 的决策模型。共享的多语言编码器和选项评分头为候选项打分，经 softmax 得到分数分布，再选择最高分选项。
...
当前路由直接选择最高分。交付要求最高分选项为 `RELEASE`，且达到配置阈值；其余情况发出一次通用复查请求。分数表示模型在各选项间的偏好。命令审批继续使用原有审批系统。

### 6.2 教师数据与联合监督微调

全部样例来源于 **GPT-6 Luna 教师模型**，再整理为模拟 EASY CODE 实际使用的用户请求与完成摘要。训练方法为 **SFT（监督微调）**：使用正确选项的交叉熵损失，同时训练编码器和选项评分头。路由与交付对训练损失的总体贡献相等。
...
关联请求按组划分，开发集与测试集约为 **4:1**。内部检查点选型使用 995 条拟合数据和 110 条验证数据，最终使用 276 条留出数据评测。

训练流程如下：

1. 从已记录版本的上游多语言权重开始，校验数据集哈希。
2. 联合训练路由和交付，让两个任务对损失的总体贡献相等，避免样例数量多的任务占主导。
3. 每个 epoch 同时打乱**样例顺序与候选答案顺序**，覆盖不同的选项位置。
...
[微调说明](../finetuning/laya-joint-v2/README.md)提供上游 GitHub／Hugging Face 地址、固定下载版本和训练／评测命令。随包提供的最终权重保存在 `model-weights/laya-multilingual/joint-v2/model/`，旁边保留结果报告与训练元数据。安装程序会准备日常决策所需的最终模型。
...
| `laya_delivery_release_threshold` | `0.9` | 交付 `RELEASE` 的最低分数；路由选择最高分选项 |

项目内 `.easycode/decision-traces/<thread-id>.jsonl` 保留实际使用的脱敏／裁剪后输入、选项顺序与分数、原始选择与实际选择、模型身份，以及回退／挑战状态。单文件达到 8 MiB 后轮转，最多保留四份旧文件，并加入本地 Git 排除规则。记录失败会提示，编程任务继续执行。任务 Journal 保存决策引用和挑战状态。

每个任务最多一次的交付挑战在暂停／恢复后仍然有效。挑战后的下一次交付继续通过原有完成检查。推理故障时，路由回退云端；交付则提示故障，继续通过原有完成检查。
...
这里对微调前基线和微调后 Laya 均按最高分选项统计：105 条路由样例遍历六种选项顺序，171 条交付样例遍历两种顺序。运行时的 0.9 交付阈值在模型预测后应用。矩阵分别包含 630 和 342 次排列评测。微调后 Laya 在所有排列均正确的样例数分别为路由 95/105、交付 110/171。

另一个独立的 [200 条微调后 Laya + GLM 实验](<../laya-bench mark/README.md>)，对两个任务都在微调后 Laya 最高分低于 0.9 时交给 GLM。整体准确率为 **88.5%**，GLM 单独判断为 **91.5%**；云端 Token 减少 **84.6%**，由 76,006 降至 11,716。该实验复用了对应样例已记录的 GLM 答案和用量。当前产品的路由选择最高分选项，交付对 `RELEASE` 应用 0.9 阈值；上面的 Token 比较属于该实验。
