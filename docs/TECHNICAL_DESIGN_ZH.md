...
项目是一个逻辑归属实体，可以先创建而不添加本地文件夹。一个项目可包含多个相互独立的文件夹根目录，并指定其中一个为主文件夹；至少添加一个文件夹后项目才能运行。项目显示名称和稳定的文件夹键都只是元数据，不会重命名源码目录。同一项目内禁止父子目录重叠，确保一个物理文件只有一个项目相对身份。

每个对话必然属于一个项目，并拥有自己的历史、所选模型、工作状态和标题。每轮对话记录项目当时的文件夹版本。项目空闲时可以修改文件夹，但只要旧版本仍可能被当前对话、命令、计划决策或委派任务观察，就禁止修改。CLI 启动时创建项目并添加启动目录；网页可以先创建空项目，再添加文件夹。

用户或主 Agent 可以为尚未命名的对话命名一次，两者遵循同一规则，避免 Agent 随后覆盖用户名称。命名能力不修改项目文件，因此主 Agent 在各种工作模式下都可使用。

同一项目中的多个对话共享全部已添加文件夹和沙箱边界。应用协调可以减少冲突写入，却不会把这些对话变成独立文件环境。多文件夹项目使用带文件夹键的命名空间路径，避免工具混淆不同根目录。单文件夹项目仍可使用子 Agent Worktree；多文件夹项目使用共享隔离，因为一个 Worktree 无法忠实代表任意多个独立仓库。

删除按照数据归属处理。删除对话会连同子对话树和相关记忆贡献一起清理；适用时，共享记忆可恢复到更早状态。删除项目还会删除其对话、项目记忆、项目 Skill 和项目自有运行数据。移除文件夹或删除项目都不授权删除用户源码目录。
...
文件工具在应用修改前检查逻辑项目添加的全部根目录及先前观察到的版本。命名空间路径禁止通过 `..` 跨越根目录，规范路径检查也会拒绝链接逃逸。并发变更应报告冲突，不能静默覆盖。命令工具具有受监督的生命周期，包括长任务、取消与清理。

Skill 是可复用的说明和配套资源，不是执行权限，也不是对话记忆。全局和项目 Skill 位于应用自有存储中，不放入某个源码文件夹。Agent 先发现描述，再按需读取说明，并通过获批操作维护内容。普通删除会归档受管理 Skill；删除项目时会删除该项目的 Skill 存储。
...
普通执行使用操作系统原生隔离。项目权限配置只开放当前项目的准确文件夹集合，拒绝写入项目外部，并单独管理网络访问。Linux 中受监督的 thread 级服务会让长期服务器和后续命令复用同一沙箱，从而支持 localhost 联调而不开放外网。沙箱无法初始化时，不会自动切换到宿主直接执行。Git Worktree 隔离的是代码变更，不是操作系统权限。
...
全局和项目记忆相互区分。项目记忆跟随逻辑项目身份，而不是某个文件夹路径；因此同一项目的所有对话和文件夹共享项目知识，不同项目仍相互隔离。写入需要校验，依赖来源的记忆可能失效，记忆也可以过期。后台整理面向已经保存的记忆，不会自动把每条消息都变成事实；整理可能额外消耗模型 Token。语义检索不可用时，可退回文本检索。
...
对话事件、结构化记录和制品在本地保存，并有明确归属。项目、带版本的文件夹关系、项目名、对话标题和 UI 偏好复用应用已有存储，不引入独立前端数据库。项目记忆和 Skill 具有确定归属，因此删除项目时无需检查或修改源码文件夹即可清理。
