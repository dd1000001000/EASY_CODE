...
- **进程约束：**获准命令的整个进程树在操作系统强制的 `workspace-write` 边界内运行，而不是继承不受限的宿主机访问权限。
...
EASY CODE 固定并复用 `@anthropic-ai/sandbox-runtime` `0.0.74` 作为命令进程沙箱，因此最低运行环境提高到 Node.js `>=20.11.0`。命令策略与审批决定操作能否启动，操作系统沙箱则独立限制获准进程树可以访问的资源。沙箱准备采用失败关闭：不支持或未初始化的后端、缺失依赖以及包装失败都会阻止执行，绝不会回退到宿主机直接启动。
...
flowchart TB
    ...
    Commands --> Sandbox[Anthropic SRT workspace-write 沙箱]
...
| 能力边界 | 把文件、命令、Plan、任务、记忆、上下文和子 Agent 操作表示为经过校验的结构化能力；命令审批与操作系统沙箱始终是两层独立强制边界。 |
...
交互式命令状态只在当前进程中生效，共有三种。手动批准保留普通决策路径；自动批准只去掉策略本来允许审批的命令提示；危险模式是需要两次明确确认的紧急状态，会把包括 Git、网络、系统、解释器和 Shell 在内的命令策略结果替换为允许。可重绘状态栏中持续显示的红色 EASY CODE 标识用于提醒用户，这项额外权限会一直保留到进程退出或主动切回其他状态；固定在 scrollback 中的会话标题不会因状态切换而重复提交。三种状态都不能关闭操作系统沙箱。

危险模式只改变命令策略，不会扩大读取、新建、更新、写入或删除文件等内置工具的边界；启动后的命令也仍位于同一个操作系统强制 `workspace-write` 文件系统边界。命令解析继续要求结构化程序与参数，工作目录必须位于当前物理根目录内，并保留受控环境、取消、超时、输出限制、进程清理、快照和审计。危险模式可以修改工作区内受保护的 Git 元数据，并获得开放网络目标，因此仍需持续显示红色警告。

子进程只获得少量允许的环境变量。命令具有取消、超时、进程树清理、输出上限、敏感信息遮蔽以及执行前后工作区快照。

永久命令分类只针对顶层调用。获准的 Shell、解释器或项目程序仍可启动后代进程，但 SRT 会把文件系统与网络边界应用到整个包装后的进程树。工作区快照仍只是有限审计辅助，不是完整监控或回滚：它会忽略 Git 元数据、依赖目录和私有状态目录，也可能因文件数量上限而截断。

### 操作系统命令沙箱

生产环境中的命令始终通过固定版本的 Anthropic Sandbox Runtime，而不是测试专用的无沙箱后端。模型提出的操作仍是结构化可执行程序与参数数组；可信 Worker 把已经解析的调用转换成 SRT 平台包装器，使用不透明命令 ID 归属违规记录，并且不会把模型文本重新解释为 Worker 自己的参数。父进程、Provider 请求、记忆数据库、凭据存储、终端 UI 与任务编排都位于命令沙箱之外。

无论命令策略处于 Manual、Auto approve 还是 Unrestricted，有效文件系统配置都固定为 `workspace-write`：
...
- `.easycode` 以及“正在被自身编辑”的 Runtime checkout 始终禁止命令写入；普通策略下 `.git` 也受保护，只有明确二次确认的危险命令策略可以写入它。
- 除父进程构造受控环境外，沙箱还会显式拒绝 Provider API Key 环境变量。
- Unix Socket、本地端口监听、Apple Events、弱化嵌套隔离和弱化网络隔离保持关闭。

网络权限来自命令能力分类，而不只取决于审批 UI。普通命令使用空的严格 Allowlist；受约束的精确版本 npm Registry 安装只获得 `registry.npmjs.org`；显式获批的 Shell 和危险命令策略获得 `*`。这样保留了受支持的依赖安装流程，但允许域名不等于允许特定操作：Shell 或危险模式仍可能发送进程能够读取的工作区数据。

沙箱初始化与执行采用失败关闭。`prepare` 或 Worker 初始化失败会形成明确的 `sandbox_unavailable` 命令结果，不会通过宿主后端重试解析后的目标程序；Host Backend 只存在于针对性的单元测试中。`/permissions` 会报告当前后端和策略，`easy-code sandbox doctor` 用于检查平台前置条件。

| 平台 | SRT 后端与前置条件 |
| --- | --- |
| macOS | 基于 Seatbelt 的 SRT；需要 `ripgrep`。 |
...
Windows 后端使用一个机器级沙箱身份和 Session 级 ACL 授权，因此 EASY CODE 会在同一进程内串行执行 Windows 沙箱命令，避免并发子 Agent Worktree 得到重叠授权。SRT 仍是 Beta Research Preview，安装在用户目录的工具链或依赖更广 Profile 访问的程序可能不兼容。后端依赖消失、UAC 初始化被取消或 Worker 清理异常都会被报告，不能被解释为允许宿主机直跑。
...
并发额度随思考强度变化：none/low 最多两个，medium 最多四个，high 最多八个。Worktree 降低子 Agent 之间的文件冲突；共享模式变更仍会串行并进行版本校验。Worktree 本身不是安全边界，但主 Agent 与子 Agent 的每条命令都会另外包装进 SRT `workspace-write` 沙箱，并以该 Agent 当前物理工作区作为根目录。Windows Alpha 后端使用同一个机器级沙箱身份，因此命令执行会被串行化；这与更高的模型工作并发额度相互独立。
...
- 策略与审批决定命令能否启动；批准后由 SRT 提供另一层操作系统边界，两层不能互相替代。
- 托管 Worktree 隔离 Git 工作状态，不自行提供安全边界；SRT 会另外把命令进程树限制在当前物理工作区。
- 固定版本的 `@anthropic-ai/sandbox-runtime` 仍是 Beta Research Preview，Windows 后端为 Alpha；平台前置条件及部分用户目录工具链可能影响兼容性。
- 网络隔离按域名而不是请求内容执行；获准 Shell 与危险命令会获得全部域名，可能发送可读取的工作区数据。
- Windows 沙箱命令会在同一 EASY CODE 进程内串行，以避免通过 SRT 共享机器身份产生重叠 ACL 授权。
