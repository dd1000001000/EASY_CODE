...
Auto 仍由模型选择，但路由模型会收到当前 Plan 与 Code 实际可用能力的权威分类摘要，例如项目调查、公开网页读取、图片理解、实施修改、外部服务和 Agent 编排，而不会收到普通工具 Schema。需要实时信息或工具的请求进入 Code；需要在实施前先审阅方案的请求进入 Plan；只有完全依据现有对话即可回答时才直接结束。无法确定时默认选择 Code。
