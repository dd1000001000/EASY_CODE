...
按 [README 安装步骤](../README_zh.md#安装) 准备 Node.js 20.11+、npm 和源码安装所需的 Git。用于子 Agent 隔离的 worktree 也依赖 Git。

安装会准备 Prompt Bundle、本地检索资源、SQLite 运行资源、可用的 VS Code 集成和匹配版本的原生沙箱。首次安装可能需要下载较大的向量模型；Windows 沙箱初始化可能请求管理员授权。安装并不保证能自动解决所有系统依赖、权限或组织安全策略问题。
