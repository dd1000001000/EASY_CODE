...
4. Harbor keeps its outer provider-endpoint allowlist and now also requires the inner command sandbox. Setup installs bubblewrap/socat/ripgrep and runs `sandbox doctor` before model work. Network restoration for the trusted verifier requires a returned agent process and no unfinished command leases or cleanup quarantine.
