...
| `provider_stream_idle_timeout_ms` | 60,000 for every effort | Renewable idle deadline for streamed main-agent requests |
...
All Runtime model calls—including the main agent, child agents, reviewers, approval, routing and compaction—prefer the same streaming provider transport. They use a renewable idle deadline selected from `limits.provider_stream_idle_timeout_ms` (60 seconds for every effort by default). Response headers and received bytes prove activity and restart that deadline, so a long reasoning or large tool-call response may run longer than one minute while it continues making progress. Endpoints that do not declare streaming support fall back to a buffered response with the fixed total deadline in `limits.provider_buffered_timeout_ms` (5/5/7.5/10 minutes). Only main-agent deltas are presented in the shared CLI; private auxiliary streams are assembled internally. Cancellation and shared task/request budgets still apply to both transports.
