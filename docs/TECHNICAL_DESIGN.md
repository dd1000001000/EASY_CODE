...
Approval is bound to the exact executable, arguments, working directory, and relevant project material. Descriptive intent does not influence classification. Disabling prompts turns approval-requiring commands into denials; automatic approval does not change direct policy denials or Plan restrictions. A user-approved shell script remains capable of performing everything expressed by that script and must be reviewed as one high-risk unit.
