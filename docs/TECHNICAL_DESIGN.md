...
Installation, engine calls, workers and uninstall share an absolute `PODMAN_CONNECTIONS_CONF` resolved by `podmanEnvironment()`. Defaults follow Podman's platform locations: `%APPDATA%/containers/podman-connections.json` on Windows, and `$XDG_CONFIG_HOME/containers` or `$HOME/.config/containers` elsewhere. Explicit host overrides are preserved and relative paths rejected. Missing Windows APPDATA falls back to the user profile. This setting is never given to model commands.

A running VM is not sufficient readiness. Setup checks the rootless VM, SSH user/port/identity, obtains its UID with `machine ssh ... id -u`, and verifies the named connection against that endpoint. Only a missing connection is restored with `system connection add`; a conflicting existing destination is never overwritten. Transient service-not-ready errors allow bounded, read-only info probes within `podmanControlTimeoutMs`, not installation or command replay.

`podman-setup.lock` serializes npm, interactive and explicit setup, with `podmanControlTimeoutMs` bounding lock acquisition. Readiness is rechecked after acquiring the lock; uninstall waits for active setup owners. Diagnostics retain the failed stage and the same process's connections file, machines and connection names rather than stale initial missing-dependency text. Set `EASY_CODE_TEST_REAL_PODMAN=1` to run the isolated real-engine connection-recovery test: it forbids installing/starting/stopping/removing actual machines and verifies the real connections file is unchanged.
...
`easy-code sandbox resources` lists EASY CODE-owned containers, review volumes and images. `easy-code sandbox remove <container|volume|image> <full-name> --yes` permanently removes one explicitly selected resource after checking its name, engine labels, control directory and lease. Running/unknown resources, outstanding leases and container references prevent forced deletion. There is no global prune or automatic deletion of machines, base images, project files or historical logs. Full CLI uninstall follows the separate confirmed workflow below; it does not use this maintenance command as a global prune. Ordinary stop retains task dependencies for resume.
...
### 7.5 Full current-user uninstall

[uninstall/](../src/uninstall) builds a read-only action plan separately from normal Agent/model-registry initialization. The uninstall branch runs before user model validation, so an invalid registry cannot prevent maintenance or cause --dry-run to recreate configuration. --data-only has been replaced by the explicit --keep-cli option.

Uninstall preflight does not require a named `--connection` or invoke setup to repair one. On Windows/macOS it derives the local SSH URI/identity from verified VM metadata and its rootless UID, then inventories through explicit `--url` / `--identity`, without creating a connection file. Setup and uninstall share [podman-connection.ts](../src/sandbox/podman-connection.ts). A foreign same-name alias still blocks removal, but a verified stale forwarding port is not foreign identity. Before mutations, VM identity, port/user, rootless status, aliases and resource inventory are checked again. Unavailable SSH credentials or an unreachable engine preserve data and report an error; they never authorize blind deletion.

The lifecycle separates stable VM identity (name, creation time, configuration directory and SSH key) from mutable connection indexes. For a surviving VM, an alias with the same key, loopback host, SSH user and socket path but an old port can be reconciled only when the writable CLI entry matches the selected registry and that old port does not belong to another Podman machine. Setup updates only the verified aliases and restores missing root/rootless aliases, preserving defaults, VM, images and keys. Uninstall is simpler: preview classifies without writing, resource inspection uses the current VM endpoint, and confirmed removal retains the actual alias snapshot for exact partial-cleanup checks. It does not recreate or retarget aliases merely to delete them. VM recreation uses the latest identity receipt rather than the first historical receipt. Windows identity-path comparisons share long/short-path normalization.

[sandbox/podman-machine-state.ts](../src/sandbox/podman-machine-state.ts) is shared by installation and uninstall. A surviving VM is inspected and reused; missing rootless/root aliases are restored from its verified endpoint. When the VM is absent (also confirmed against WSL on Windows), verified stale aliases are removed before installation may initialize a new machine. Uninstall uses the same identity checks, without initializing a VM. Connections with foreign keys/endpoints, a live provider VM, unknown writability or inconsistent metadata stop the operation before destructive changes. Podman can also remove the VM/configuration and then report a missing alias: only that precise partial-removal error is reconciled automatically, after verifying provider removal. No mutation is blindly replayed; each alias deletion rechecks current state and never uses global remove/prune/reset. `EASY_CODE_TEST_REAL_PODMAN_CONNECTIONS=1` enables real-CLI cleanup tests against isolated registry files and asserts that the actual user's registry remains unchanged.

Orphan identity comparisons resolve existing path ancestors, including Windows short/long aliases when the key file has already been deleted. Rejections identify the failed writability, identity, endpoint/role, receipt or paired-port check. CLI/registry disagreement is a distinct inventory-source error, with the selected file and endpoint comparison; diagnostics never read private-key contents.

`IsMachine` is a creation hint, not an ownership contract: Podman's ordinary `system connection add` does not set it. Setup records `machine-connection` receipts bound to the exact registry, machine name, URI and key path after verifying both aliases. Recovery first uses these host-written receipts; legacy installations without receipts require the standard Podman machine-key path, matching writable registry entry, exact machine alias, loopback SSH and correct root/rootless socket role. Missing or false `IsMachine` alone does not invalidate this independent evidence; malformed field types are rejected. Missing, unreadable, redirected or inconsistent registry metadata still blocks removal. Setup, runtime and uninstall capture their control environment so a transaction cannot drift across connection files. Test runners must supply their own registry rather than borrowing host evidence. Lifecycle tests cover fresh installation, ordinary connection repair, partial removal, reinstall, source mismatch and foreign-resource preservation.

[install/ownership.ts](../src/install/ownership.ts) records host-created directory locations, machine identities, base-image IDs, package-install provenance and credential slot names in the private install-resources.jsonl inventory. No secret values are recorded. Existing installations are discovered from fixed locations, user configuration and Runtime metadata; project configuration cannot supply deletion targets. Custom data roots require an ownership marker. Unknown files remain visible as preserved entries; ancestors that are symlinks/junctions are rejected and leaf links are unlinked without traversing their targets.

The confirmed workflow obtains a maintenance lock, prevents new CLI sessions, requests cooperative shutdown, waits for thread/command/snapshot owners to finish, then removes sandbox resources, registered detached Worktrees, the extension, credentials, data/cache/configuration and finally the npm package. A shutdown timeout or ambiguous owner stops cleanup; arbitrary PIDs are never killed. Podman inspection verifies rootless operation and resource ownership. A stopped dedicated machine is started only after confirmation so ownership can be inspected; foreign contents reject removal. Shared Linux engines delete only identified resources, without reset/global prune. Windows WinGet/macOS Homebrew Podman installs may be removed only with installation provenance and no remaining machines/connections; pre-existing software, WSL, Node, Git, and unsupported automatic software-uninstall recipes are preserved and reported.

The CLI presents a compact scope and asks once for `y`; `--yes` grants the same full-plan consent without prompts, while `--dry-run` prints every exact target without mutations. Consent includes verified legacy machines, old native-sandbox temporary state, corrupt stores whose legacy leases cannot be checked (all sessions must be closed first), and managed Worktrees with dirty/unintegrated changes. The CLI supplies the complete preview's internal consent tokens to the executor; there is no per-resource confirmation prompt or `--confirm-resource` option. Ownership, active-owner and stale-plan checks are not bypassed. Progress is grouped, while preserved entries and errors remain visible. Worktrees are checked against Git's NUL-delimited inventory and Runtime path layout, removed through Git, and only their Runtime namespace refs are deleted; user branches and main checkouts are untouched. Temporary review copies require a matching review ID, snapshot hash and local author/reviewer roots in `binding.json`. Real paths normalize Windows short/long aliases; linked or mismatched roots are still preserved rather than traversed.

Execution persists phase completion and resource discovery hints in the per-user .easy-code-uninstall-state.json file, outside the directories being deleted. Failure stops before later phases, returns a nonzero result and keeps recovery information; another invocation can continue. A crash leaving an ownership-ambiguous maintenance lock requires inspection rather than automatic lock erasure. npm removal is pinned to the verified global prefix and runs last with lifecycle scripts disabled; a global junction to source is removed by npm, never recursively through its target. No real workstation uninstall is performed by the automated tests.
