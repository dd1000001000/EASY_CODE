...
该选择会同时缩放有效短期上下文的字符上限，以及每条用户任务最多可以进行的模型请求步数。可配置的基础值用于 `none` 和 `low`；`medium` 乘以 2，`high` 乘以 4。使用默认基础值时，各档如下：

| 思考强度 | 倍率 | 有效上下文上限 | 步骤上限 |
| --- | ---: | ---: | ---: |
| `none` | 1× | 400,000 字符 | 40 |
| `low` | 1× | 400,000 字符 | 40 |
| `medium` | 2× | 800,000 字符 | 80 |
| `high` | 4× | 1,600,000 字符 | 160 |

即使所选模型不支持思考参数，这两个 Runtime 预算仍然按照 Thread 中保存的档位生效。可以在用户配置或 `.easycode/config.toml` 中配置 `none`/`low` 的基础值：
...
对应的环境变量是 `EASY_CODE_MAX_STEPS` 和 `EASY_CODE_MAX_CONTEXT_CHARS`。它们现在都表示基础值，因此有效值为“基础值 × 思考强度倍率”。

迁移提示：旧版本把 `max_steps` 作为硬上限，并把 `max_context_chars` 作为所有思考强度共用的绝对上限。升级后，已有的显式配置值会参与倍率计算。请检查或移除这些设置；如果只需为某个档位保留指定的有效上限，可把新基础值设为“目标有效上限 ÷ 该档倍率”。
...
主提示符会显示短期记忆的估算 Token 数，例如 `context:12.4k`。统计范围包括持久化上下文摘要、当前有效的对话与工具消息、返回到上下文中的 Thinking 内容以及有效图片的 Token 估算。由于不同模型的 tokenizer 不同，该数值是近似值；它只用于显示，不是触发上下文压缩的字符计量。
...
- 上下文由字符、输出和图片预算控制。有效短期上下文的字符上限由当前思考强度派生：默认的 `none`/`low` 基础值为 400,000 字符，`medium` 为 800,000，`high` 为 1,600,000。Runtime 统计持久化 Working Summary 与当前压缩边界之后的消息，不计入已经压缩的原始历史。压力百分比以当前档位的有效上限为分母，而不是未缩放的基础值，并按四档处理：

  - 低于 60%：不干预。
  - 达到 60%：提示模型考虑调用 `compact_context`，正常工作仍可继续。
  - 达到 80%：要求模型下一步只能单独调用 `compact_context`。
  - 达到 90%：Runtime 自动注入强制压缩请求，并继续把该步限制为只能单独调用 `compact_context`。

  `compact_context` 会生成累计的 `workingSummary` 并推进持久化压缩边界。如果单次请求仍超过当前有效字符硬限制，请求级兜底机制可以截断实际发送给模型的内容，但不会更新 `workingSummary`，也不会推进压缩边界。终端中的 `context:…` Token 数仍然只是与 tokenizer 无关的近似显示，不参与这些字符阈值的判断。
