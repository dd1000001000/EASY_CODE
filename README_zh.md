...
- 模型处理请求时仍可提交文字或图片调整，并在下一个安全模型边界统一应用所有待处理消息。
...
选择 Yes 会切回 Auto 并执行已经接受的方案；选择 No 会拒绝方案；选择反馈行后会打开支持安全粘贴与多行内容的普通输入框，再让模型调整方案并重新提交。
...
Resume 会尽可能恢复已经保存的状态，包括所选模型和模式、对话、尚未应用的 Turn 中途调整、已接受计划、任务进度、文件与命令历史、上下文摘要、已完成的子 Agent 结果，以及仍然有效的活动子 Thread/执行环境绑定。关键身份、仓库和路径元数据校验通过，并且已保存的快照 Commit 仍可解析时，缺失的托管 checkout 目录才能重建。已有子 Agent 的环境记录缺失，或其中的身份、仓库、路径元数据校验不一致时会失败关闭，不会另建 checkout 后静默改绑。已经持久完成的子任务不会重跑；被中断的命令或模型调用也不会自动重复执行。
