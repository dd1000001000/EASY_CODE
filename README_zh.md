...
应用和历史记录保存在本地，但完成任务所需的上下文仍会发送给所选模型供应商。“本地运行”不代表离线模型推理。

## 安装

需要 **Node.js 20.11+**、npm，以及可用的模型供应商账号。以下源码安装方式还需要 Git；原生沙箱是否可用取决于操作系统和架构。
...
请确认每一步成功后再执行下一步。全局安装会准备本地检索资源、编辑器集成和原生沙箱，可能需要下载资源；Windows 可能请求管理员授权。日常使用不需要 Docker、Podman，也不需要另外安装 Codex 应用。
...
网页中先创建项目、关联本地文件夹，再点击项目旁的 **＋** 创建对话，使用输入框下方的控件选择模型。请保持启动终端打开；网页服务仅供本机访问。
...
希望先看方案时选择 Plan；直接实施选择 Code；也可以使用 Auto 自动判断。**Plan 是工作方式，不是强制只读的安全边界。**
