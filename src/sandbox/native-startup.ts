...
import { nativeSandboxEntrypoint, nativeSandboxHome, nativeSandboxRuntimeVersion } from "./native-runtime.js";
...
export class NativeSandboxStartupService implements SandboxStartupService {
  ...
  async setup(readiness?: SandboxReadiness): Promise<SandboxSetupResult> {
    ...
    const home = nativeSandboxHome(this.dataDir); await mkdir(home, { recursive: true, mode: 0o700 });
    ...
    try {
      ...
      return { status: "failed", message: error instanceof Error ? error.message : String(error), readiness: after };
    } finally { await service.close(); }
    const after = await this.inspect();
