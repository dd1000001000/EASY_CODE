...
export class BenchmarkContainerBackend implements CommandExecutionBackend {
  ...
  quarantine(reason: string): void { BenchmarkContainerBackend.failed.abort(new Error(reason)); }
  ...
  async prepare(request: SandboxExecutionRequest): Promise<PreparedCommand> {
    ...
    const release = await BenchmarkContainerBackend.acquire(request.context.signal);
    ...
    await writeFile(path.join(dir, "request.pending"), JSON.stringify({ version: 1, commandId: request.commandId,
      ...
      environment: request.command.environment, timeoutMs: 1200000 }), { flag: "wx", mode: 0o600 });
    ...
    return { executablePath: process.execPath,
      ...
      cwdAbsolute: "/", environment: { PATH: "/usr/bin:/bin" }, metadata: this.describe(), controlPipe: true,
      cooperativeTermination: true,
      cleanup: async () => { try { await rm(dir!, { recursive: true, force: true }); } finally { release(); } } };
    } catch (error) {
