...
function assertTaskGraphInvariants(graph: TaskGraph): void {
  ...
  if (inProgress.length > 1) throw new Error("Only one task may be in progress");
  ...
  const allCompleted = graph.tasks.every((task) => task.status === "completed");
  const hasBlocked = blocked.length === 1;
  const expectedStatus: TaskGraphStatus = allCompleted
    ? "completed"
    : hasBlocked
      ? "blocked"
      : "active";
...
export function applyTaskGraphOperation(
  ...
  if (operation.action === "start") {
    ...
    if (graph.tasks.some((candidate) => candidate.status === "in_progress")) {
      throw new Error("Complete or block the current task before starting another one");
    }
    ...
    if (operation.evidence.length !== task.completionChecks.length) {
      throw new Error(
        `Task ${task.id} requires exactly ${task.completionChecks.length} completion evidence item(s)`,
      );
    }
    if (
      operation.evidence.reduce((total, evidence) => total + evidence.length, 0) >
      MAX_TASK_COMPLETION_EVIDENCE_TOTAL_CHARS
    ) {
      throw new Error(
        `Task ${task.id} completion evidence exceeds ` +
    ...
    for (const evidence of operation.evidence) assertSafeText(evidence);
    task.completionEvidence = task.completionChecks.map((check, index) => ({
      check,
      evidence: operation.evidence[index] as string,
    }));
    task.status = "completed";
    task.completedAt = now;
    graph.status = graph.tasks.every((candidate) => candidate.status === "completed")
      ? "completed"
      : "active";
    ...
    if (graph.status !== "blocked" || task.status !== "blocked") {
...
export function taskGraphView(graph: Readonly<TaskGraph>): TaskGraphView {
  const byId = new Map(graph.tasks.map((task) => [task.id, task]));
  const current = graph.tasks.find((task) => task.status === "in_progress");
  const noCurrent = current === undefined;
  ...
  return {
    ...
    startableTasks: graph.status === "active" && noCurrent
      ? tasks
...
export function activeTask(graph: Readonly<TaskGraph> | undefined): TaskNode | undefined {
  return graph?.tasks.find((task) => task.status === "in_progress");
}
