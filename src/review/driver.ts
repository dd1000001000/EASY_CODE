...
const reportTool: ToolDefinition = { type: "function", function: { name: "submit_review_result",
  description: "Finish this independent review with one attributed conclusion and a concrete next action. No vote, consensus, or approval. Cite only captured evidence IDs; mark unverified points as uncertainties.",
  ...
  }, required: ["conclusion", "nextAction", "evidenceRefs", "uncertainties"] } } };
...
export function createReviewDriver(input: ReviewDriverInput): { investigate(): Promise<ReviewReport> } {
  ...
  const systemPrompt = "You are an independent reviewer. The main Agent is paused while you investigate. " +
    ...
    "Do not edit the real checkout or project memory. Submit one conclusion via submit_review_result alone; there is no agreement round or Runtime correctness certificate.\n\n" +
