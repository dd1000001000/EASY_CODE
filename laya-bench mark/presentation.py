...
LABELS = {"Laya": "Laya", "GLM": "GLM", "Cascade": "Laya + GLM"}
...
def render_report(folder: Path, rows: list[dict], metrics: dict, threshold: float) -> None:
    ...
    with plt.rc_context({"font.family": "DejaVu Sans", "font.size": 11,
        ...
        fig.text(.065, .955, "Laya + GLM", fontsize=31, weight="bold")
        fig.text(.065, .918, "Local decisions. Selective cloud fallback.", fontsize=15, color=MUTED)
        fig.text(.065, .883,
        ...
        token_ax = fig.add_axes([.125, .14, .445, .202])
        ...
        token_ax.set_yticks([1, 0], ["GLM", "Laya + GLM"])
        ...
        for spine in token_ax.spines.values():
            spine.set_visible(False)
        fig.text(.065, .098, "Laya-only uses 0 cloud tokens.", fontsize=10, color=MUTED)
        ...
        fail_ax.set_xticks(range(3), [LABELS[method] for method in METHODS])
    ...
    lines = ["# Laya + GLM", "",
             f"100 routing + 100 delivery cases. Laya decides first; confidence below **{threshold:.2f}** sends the case to GLM.",
    ...
    lines += ["", f"The cascade cuts cloud tokens by **{savings:.1%}** and GLM calls from **{total} to {len(forwarded)}**. "
              ...
              f"**Laya {false_releases['Laya']} · GLM {false_releases['GLM']} · Laya + GLM {false_releases['Cascade']}**.",
