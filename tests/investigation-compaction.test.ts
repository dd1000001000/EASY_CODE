...
import { completeExchange, retirementBoundary, foldCompactionControl, investigationExchangeStart, runCompactionTransaction } from "../src/context/compaction-transaction.js";
...
function fixture(rounds = 6, tokens = true, recordBoundaries = true, resultChars = 7000, reasoningChars = 7000) {
    ...
    function message(m, extra = {}) {
        ...
        if (m.role === "tool")
            foldPendingOperations(state, { tool: m.name, ...extra });
    }
    if (rounds)
        message({ role: "user", content: "Investigate and fix the parser" });
    ...
    const run = (overrides = {}) => runCompactionTransaction({
        ...
        state, manager, limits: fixtureLimits, forceRecovery: true, turnId: "turn", maxContextChars: 100_000, required: true,
...
describe("unfinished investigation compaction", () => {
    ...
    it("references a single oversized tool result without clipping its thinking or changing the protocol", async () => {
        ...
        try {
            ...
            assert.ok((recalled?.data).totalChars > 100_000);
    ...
    it("archives an oversized previous summary when repair and conservative recovery cannot fit", async () => {
        ...
        try {
            ...
            await f.run({ limits: { ...fixtureLimits, contextSummaryMaxTokens: 12000 }, complete: async () => largeCandidate });
            ...
            const result = await f.run({ maxContextChars: 50_000, maxAttempts: 1,
                complete: async () => ({ role: "assistant", content: "invalid" }) });
            assert.equal(result.committed, true);
            ...
            assert.equal(recalled?.ok, true);
            assert.equal(JSON.parse((recalled?.data).text), oldSummary);
    ...
    it("compacts with a running command and preserves its exact polling identity across Resume", async () => {
        ...
        try {
            ...
            const result = await f.run({ maxContextChars: 40_000 });
            assert.equal(result.committed, true);
            ...
            f.message({ role: "tool", name: "poll_command", tool_call_id: "poll", content: "finished" }, { contextCommand: { ...command, status: "exited", exitCode: 0 } });
    ...
    it("pins standalone child requirements and follow-ups without freezing history", async () => {
        ...
        try {
            ...
            for (const [id, lifecycle] of [["start_child", { action: "activate", agentId: "child" }],
                ...
                f.message({ role: "tool", name: "manage_subagents", tool_call_id: id, content: "running" }, { subagentLifecycle: lifecycle, ...(lifecycle.action === "activate" ? { subagentAssignment: assignment } : {}) });
            }
            assert.equal((await f.run({ maxContextChars: 40_000 })).committed, true);
    ...
    it("recovers an eviction committed just before a crash without repeating it", async () => {
        ...
        try {
            await assert.rejects(f.run({ maxContextChars: 30_000, append: async (event) => {
                    ...
                    if (event.type === "context.history.evicted")
                        throw new Error("crash after eviction append");
    ...
    it("uses raw complete exchanges without requiring historical phase events", async () => {
        ...
        try {
            assert.equal(retirementBoundary(f.state, 2), 9);
    ...
    for (const tokens of [false, true])
        it(`compacts six read-only rounds without tests (${tokens ? "token" : "character"} mode)`, async () => {
            ...
            try {
                ...
                for (const recent of f.state.messages.slice(9))
                    assert.ok(projected.some((m) => JSON.stringify(m) === JSON.stringify(recent)));
                const summary = JSON.parse(f.state.workingSummary);
    ...
    it("prefers the retained-exchange count but can reduce the tail to release capacity", () => {
        const f = fixture();
        try {
            assert.equal(retirementBoundary(f.state, 3), 7);
            assert.equal(retirementBoundary(f.state, 6), 3);
            assert.equal(retirementBoundary(f.state, 2), 9);
            const recovered = f.store.recover(f.state.threadId);
            assert.equal(retirementBoundary(recovered, 3), 7);
        }
        finally {
            f.dispose();
        }
    ...
    it("rejects missing tool results but allows complete write batches", () => {
        ...
        try {
            ...
            assert.throws(() => foldCompactionControl(f.state, "context.phase.closed", {
                end: f.state.messages.length, kind: "investigation"
    ...
    it("keeps all calls/results in a multi-tool exchange and refuses an open call", async () => {
        ...
        try {
            ...
            const batch = [{ role: "assistant", content: null, reasoning_content: "intact batch reasoning",
                    tool_calls: ["a", "b"].map((id) => ({ id, type: "function", function: { name: "read_file", arguments: "{}" } })) },
            ...
            assert.equal(retirementBoundary(f.state), undefined);
            const result = await f.run({ append: async () => undefined });
    ...
    it("Runtime records investigation boundaries and continues after automatic compaction", async () => {
        ...
        try {
            let reads = 0;
            let summaries = 0;
            const readTool = { name: "read_file", mutating: false, definition: { type: "function", function: {
            ...
            const runtime = new AgentRuntime({ provider: { name: "deepseek", model: "test", complete: async (request) => {
                        ...
                        if (reads >= 6)
                            return { message: { role: "assistant", content: "Investigation remains unfinished; reproduce the failure next." } };
                        reads += 1;
            ...
            assert.ok(f.events.some((e) => e.type === "context.phase.closed" && e.payload.kind === "investigation"));
