...
import { completeExchange, retirementBoundary, foldCompactionControl, prefixHash, runCompactionTransaction } from "../src/context/compaction-transaction.js";
...
function candidate(_primary = 0, text = "Fix the task") {
    return { role: "assistant", content: null, tool_calls: [{ id: "compact_candidate", type: "function",
                function: { name: "compact_context", arguments: JSON.stringify({
...
describe("completed-phase compaction transactions", () => {
    ...
    it("preserves the complete normal prefix and schema order across summary corrections without dispatching tools", async () => {
        const f = fixture();
        const normal = { ...envelope, tools: ["run_command", "read_file"].map(name => ({
        ...
        let calls = 0;
        try {
            const result = await f.run({ nextRequest: normal, complete: async (messages, attempt, tools) => {
                    calls++;
                    ...
                    if (calls === 2)
                        assert.match(messages.at(-1)?.content ?? "", /RUNTIME_SUMMARY_CORRECTION/u);
    ...
    it("recovers locally when the retained schemas make the summary request too large", async () => {
        const f = fixture();
        const normal = { ...envelope, tools: [{ type: "function", function: {
        ...
        for (const failedApi of [false, true]) {
            ...
            try {
                const result = await f.run({ complete: async () => {
                        calls++;
                        if (calls === 1)
                            return { role: "assistant", content: "Earlier unverified handoff" };
                        if (failedApi)
                            throw new ProviderError("busy", { provider: "deepseek", code: "503", retryable: true });
                ...
                if (failedApi)
                    assert.equal(f.events.filter(e => e.type === "context.compaction.rejected").length, 1);
    ...
    it("salvages a durable earlier body when a later correction was dispatched before a crash", async () => {
        ...
        try {
            await assert.rejects(f.run({ complete: async () => ({ role: "assistant", content: "Persisted raw body" }), append: async (event) => {
                    ...
                    if (event.type === "context.compaction.attempt" && event.payload.attempt === 2)
            ...
            assert.equal(result.requests, 0);
            assert.match(resumed.workingSummary, /Persisted raw body/);
    ...
    it("clips length immediately without a correction request and retains valid siblings", async () => {
        ...
        try {
            const result = await f.run({ maxAttempts: 2, maxRequests: 2, complete: async (messages, attempt) => {
                    if (attempt === 1)
                        return candidate(0, "x".repeat(1332));
    ...
    it("never repairs invalid field types by clipping a different overlong field", async () => {
        ...
        try {
            const result = await f.run({ complete: async () => {
                    ...
                    response.tool_calls[0].function.arguments = JSON.stringify({ currentWork: "x".repeat(1300), nextStep: 42 });
                    return response;
                } });
            assert.equal(result.requests, 3);
            assert.equal(result.paused, undefined);
    ...
    it("recovers a durable candidate after commit-write failure without another model request", async () => {
        ...
        try {
            await assert.rejects(f.run({ append: async (event) => {
                    if (event.type === "context.compaction.committed")
    ...
    it("keeps verification events independent from structural retirement boundaries", () => {
        ...
        try {
            ...
            assert.equal(retirementBoundary(f.state), 2); // Default five-exchange protection.
        }
        finally {
            f.dispose();
        }
    ...
    it("a lost summary response still consumes its single durable attempt", async () => {
        ...
        try {
            const result = await f.run({ complete: async () => {
                    throw new ProviderError("network disconnect", { provider: "deepseek", code: "network_error", retryable: true });
                } });
            assert.equal(result.paused, undefined);
            const resumed = f.store.recover(f.state.threadId);
            assert.equal(resumed.compactionControl?.transaction?.attempts, 1);
    ...
    it("commits idempotently and rejects malformed events before poisoning the journal", async () => {
        ...
        try {
            await f.run();
            const commit = f.events.find((event) => event.type === "context.compaction.committed");
            ...
            assert.throws(() => f.store.appendEvent(f.state.threadId, { type: "context.compaction.started",
                payload: { id: "forged", start: 2, end: 99, sourceHash: "a".repeat(64), status: "pending", attempts: 0, maxAttempts: 1 } }), /Invalid compaction/u);
            assert.equal(f.store.recover(f.state.threadId).compactedMessageCount, 2);
        }
        finally {
            f.dispose();
    ...
    it("rejects split exchanges and stale sources but permits returned command handles", async () => {
        const f = fixture();
        try {
            const exchange = [{ role: "assistant", content: null, tool_calls: [{ id: "call", type: "function",
            ...
            assert.equal(retirementBoundary(f.state), 2);
            assert.throws(() => foldCompactionControl(f.state, "context.compaction.started", {
                id: "stale", start: 0, end: 2, sourceHash: "a".repeat(64), status: "pending", attempts: 0, maxAttempts: 1,
...
describe("provider-neutral token calibration", () => {
    const messages = [{ role: "user", content: "estimate this input ".repeat(500) }];
    it("uses total prompt usage once, persists estimates and isolates model/endpoint identities", () => {
        ...
        try {
            ...
            assert.equal(f.storage.db.prepare("SELECT count(*) AS count FROM context_token_samples").get().count, 1);
        }
        finally {
            f.dispose();
        }
    ...
    it("ignores missing/invalid usage and bounds sample retention without modifying source messages", () => {
        ...
        try {
            ...
            for (const usage of [undefined, {}, { promptTokens: -1 }, { promptTokens: NaN }, { promptTokens: 0 }])
                calibration.observe(messages, [], usage);
            assert.equal(calibration.estimate(messages), requestTokens(messages));
            for (let n = 0; n < 50; n += 1)
                calibration.observe(messages, [], { promptTokens: 1000 });
            assert.equal(f.storage.db.prepare("SELECT count(*) AS count FROM context_token_samples").get().count, 32);
    ...
    it("reserves future tools as well as output and safety at the final request guard", () => {
        ...
        assert.throws(() => budgetedRequest({ messages, tools: [] }, budget, () => budget.inputCapacity + 1), /context_capacity_insufficient/u);
        assert.equal(budgetedRequest({ messages, tools: [] }, budget, () => budget.inputCapacity).outputReserveTokens, budget.outputReserve);
    });
});
