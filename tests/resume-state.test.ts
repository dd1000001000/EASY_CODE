...
describe("resume state recovery", () => {
    it("restores only file read versions that still match the current workspace", async () => {
        ...
        try {
            ...
            const result = manager.restorePersistedState(savedReads, [historicalChange, historicalChange]);
    ...
    it("hydrates verified reads, audit state, and Thinking history through app startup resume", async () => {
        ...
        try {
            ...
            assert.deepEqual(restored.workspace.getReadVersions().map((entry) => entry.path), ["stable.txt"]);
    ...
    it("keeps the event-authoritative compaction boundary across a stale checkpoint", () => {
        ...
        try {
            ...
            state.messages.push({ role: "user", content: "one" }, { role: "assistant", content: "two" }, { role: "user", content: "three" });
            threads.save(state);
            ...
            assert.equal(recovered.workingSummary, "Durable compacted objective and verified state.");
            assert.deepEqual(recovered.messages.map((message) => message.content), ["one", "two", "three"]);
            threads.appendEvent(state.threadId, {
                type: "context.compaction.committed",
                phase: "completed",
                payload: {
    ...
    it("records interrupted-turn repair atomically and only once", () => {
        ...
        try {
            ...
            assert.equal(threads.journal(state.threadId).read().filter((event) => event.type === "turn.recovered").length, 1);
            assert.equal(recovered.activeTurnId, undefined);
            assert.equal(recovered.messages.some((message) => message.role === "tool" && message.tool_call_id === "missing_call"), true);
        }
        finally {
            storage.close();
            rmSync(dataDir, { recursive: true, force: true });
    ...
    it("returns an interrupted approved-plan execution to explicit review", () => {
        ...
        try {
            ...
            assert.equal(repairInterruptedTurn(threads, interrupted), true);
            const repairedPlan = interrupted.planReview;
    ...
    it("does not duplicate a final assistant reply that was durable before the crash", () => {
        ...
        try {
            ...
            assert.equal(repairInterruptedTurn(threads, interrupted), true);
            const recovered = threads.recover(state.threadId);
            assert.equal(recovered.messages.filter((message) => message.role === "assistant" && message.content === "The durable final answer.").length, 1);
            assert.equal(recovered.messages.some((message) => message.role === "assistant" &&
                message.content?.includes("marked as interrupted")), false);
            assert.equal(recovered.activeTurnId, undefined);
        }
        finally {
            storage.close();
    ...
    it("does not mistake an assistant followed by a Runtime reminder for a final reply", () => {
        ...
        try {
            ...
            const recovered = threads.recover(state.threadId);
            assert.equal(recovered.messages.some((message) => message.role === "assistant" &&
                message.content?.includes("marked as interrupted")), true);
        }
        finally {
            storage.close();
            rmSync(dataDir, { recursive: true, force: true });
