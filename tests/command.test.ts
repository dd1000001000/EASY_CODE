...
describe("command validation change integration", () => {
  it("marks a modified original test from recorded file changes without a full baseline", async () => {
    await withWorkspace(async (root, manager) => {
      const filename = path.join(root, "test.cjs");
      const original = "const a=require('node:assert/strict');a.equal(2,3);\n";
      const weakened = original.replace("a.equal(2,3)", "a.equal(2,2)");
